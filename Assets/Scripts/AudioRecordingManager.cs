...
namespace opal
{
    ...
    public class AudioRecordingManager : MonoSingleton<AudioRecordingManager>
    {
        ...
        public AudioClip GetChunkOfRecordedAudio(float offsetInSeconds, float chunkLengthInSeconds)
        {
            ...
            {
                Logger.LogError( "Audio chunk length cannot be less than or equal to 0.");
                return null;
            }
            if (offsetInSeconds >= m_RecordedAudio.length)
            {
				Logger.LogError("Offset cannot be greater than or equal to the recorded audio length.");
                return null;
            }
            ...
            {
				Logger.LogWarning( "Chunk offset is less than 0. Clamping to 0.");
                offsetInSeconds = 0;
            }
